...
export function EditorialFeatureCard({ post, href, label = 'Cover story' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className="group block min-w-0 overflow-hidden rounded-[2rem] bg-[#173f67] text-white">
      <div className="relative aspect-[16/10] min-h-[420px] overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 sm:p-9">
          <span className="rounded-full bg-[#f26076] px-3 py-2 text-[10px] font-black uppercase tracking-[.18em]">{label}</span>
          <h3 className="editorial-serif mt-5 max-w-4xl text-4xl font-bold leading-[.97] tracking-[-.045em] sm:text-6xl">{post.title}</h3>
...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden rounded-[1.4rem] bg-white shadow-sm ${dc.motion.lift}`}>
      <div className="relative aspect-[4/3] overflow-hidden bg-[#dff5f3]">
      ...
      <div className="p-5">
        <div className="flex items-center justify-between gap-3 text-[10px] font-black uppercase tracking-[.18em] text-[var(--slot4-accent)]">
          <span>{getEditableCategory(post)}</span><span className="grid h-7 w-7 place-items-center rounded-full bg-[#ffd150] text-[#173f67]">{String(index + 1).padStart(2, '0')}</span>
        </div>
        <h3 className="editorial-serif mt-3 line-clamp-3 text-xl font-bold leading-[1.12] tracking-[-.025em]">{post.title}</h3>
      </div>
    </Link>
  )
}
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 border-t border-black/20 py-5 first:border-t-0">
      <span className="text-3xl font-black leading-none text-[#f26076]">{String(index + 1).padStart(2, '0')}</span>
      <div className="min-w-0">
        ...
        <h3 className="editorial-serif mt-2 line-clamp-3 text-lg font-bold leading-tight tracking-[-.02em] group-hover:text-[#458b73]">{post.title}</h3>
      </div>
    </Link>
  )
}
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 gap-5 rounded-[1.5rem] bg-white p-4 shadow-sm transition hover:-translate-y-1 sm:grid-cols-[240px_minmax(0,1fr)]">
      <div className="relative aspect-[16/10] overflow-hidden rounded-[1.1rem] bg-[var(--slot4-media-bg)]">
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" />
      </div>
      <div className="min-w-0 pt-4 sm:pt-1">
        ...
        <h2 className="editorial-serif mt-3 line-clamp-3 text-3xl font-bold leading-[1.02] tracking-[-.035em] group-hover:text-[#458b73]">{post.title}</h2>
