...
function BackLink({ task }: { task: TaskKey }) {
  return (
    <Link href="/" className="inline-flex items-center gap-2 border-b-2 border-current pb-1 text-xs font-black uppercase tracking-[0.16em]">
...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-[#fffaf3]">
      <header className="bg-[#bfeaf0]">
        <div className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t-4 border-[#173f67] pt-4 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="text-[#f26076]">{categoryOf(post, 'News')}</span>
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="editorial-serif mt-6 max-w-6xl text-5xl font-bold leading-[0.94] tracking-[-0.055em] sm:text-6xl lg:text-[5.5rem]">{post.title}</h1>
          {summaryText(post) ? <p className="mt-6 max-w-4xl text-xl font-bold leading-8 text-[#173f67]/68 sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto mt-10 max-w-[1320px] overflow-hidden rounded-[2rem] bg-white shadow-[0_24px_70px_rgba(23,63,103,.14)]">
      ...
      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-[1.5rem] bg-white p-6 shadow-sm sm:p-9">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div className="border-t-4 border-[#f26076] pt-5">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[#173f67]/15 py-3 transition hover:text-[#458b73]">
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-[1.5rem] border-t-4 border-[#ffd150] bg-[#e1f4f1] p-6">
